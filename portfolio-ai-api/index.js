...
app.post("/knowledge", authenticateToken,async (req, res) => {
...
app.put("/knowledge/:id", authenticateToken, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    // Update the existing knowledge in the knowledge base
...
app.delete("/knowledge", authenticateToken, async (req, res) => {
  const { id } = req.body;

  if (!id) return res.status(400).json({ error: "ID is required" });

  try {
    // Delete the knowledge from the knowledge base
